...
async function getHeirarchy(community_id){
    // try{
        const community = await Community.findOne(
            {community_id},
            { community_id: 1, name: 1,sub_communities:1, profile_img: 1,_id:1 }
        );

        if(community.sub_communities.length===0) return community

        const sub_communities = []

        for(let sub of community.sub_communities){
            const sub_community = await getHeirarchy(sub)
            sub_communities.push(sub_community);
        }
        
        community.sub_communities = [...sub_communities];
        return community
...
router.get("/u/:user_id/get-all-heirarchy",async (req,res)=>{
    try {
        ...
        await Community.find(
            ...
            { community_id: { $in:  community_ids },parent_community:null },
            
            { community_id: 1, name: 1, profile_img: 1,_id:1 }
        ).then((data) => {
            result = data;
        });
        ...
        for (let top_community of result){
            const h = await getHeirarchy(top_community.community_id);
            heirarchy.push(h);
        }
        
        
        return res.json({ status: 200, success: true, heirarchy });
    } catch (error) {
        console.error("Unable to fetch heirarchy followed by user: ", error);
        return res.json({
            status: 500,
            success: false,
            message: "Unable to fetch heirarchy followed by user",
        });
    }
});
