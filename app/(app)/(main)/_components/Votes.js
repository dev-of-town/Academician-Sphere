...
const Votes = ({ votes, postid, isUpvoted, isDownVoted }) => {
  ...
  const countVotesRef = useRef(votes);

  const upVote=()=>{
    if(downvoted){
      downVote();
    }
    countVotesRef.current += (upvoted?-1:1);
  ...
  const downVote=()=>{
    ...
    countVotesRef.current += (downvoted?1:-1);
  ...
  const handleUpvote = async () => {
    ...
    try {
      ...
      if (!success) {
        throw Error("Post upvote is not successful");
      }
    } catch (error) {
      console.log(error);
      countVotesRef.current -= 1;
      setUpvoted(!upvoted);
    }
  };
  ...
  const handleDownvote = async () => {
    downVote();
    try {
      const response = await fetch(`/api/downvote/${postid}`, {
        method: "POST", // *GET, POST, PUT, DELETE, etc.
        mode: "cors", // no-cors, *cors, same-origin
        headers: {
            "Content-Type": "application/json",
      ...
      const { success } = await response.json();
      if (!success) {
        throw Error("Post downvote is not successful");
      }
    } catch (error) {
      console.log(error);
      countVotesRef.current += 1;
