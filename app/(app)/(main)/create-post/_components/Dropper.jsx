...
const Dropzone = ({ className, crt_post, changeCon, formData }) => {
  ...
  const removeFile = (name) => {
    let d = formData.getAll("attachement");
    // let i = d.findIndex((obj)=>{
    //    return obj.name === name;
    // });
    // if(i>=0){
    //    d.splice(i,i+1);
    //    formData.set("attachement",d);
    // }
    setFiles((files) => files.filter((file) => file.name !== name));
    // console.log("-->",formData,d,formData.getAll("attachement"));
  };

  const removeAll = () => {
    formData.delete("attachement");
    setFiles([]);
    setRejected([]);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    // files.forEach((file) => {
    //   console.log(file);
    //   formData.append("attachement",files)
    // });
    files.forEach((file)=>formData.append("attachement", file));
    console.log("get attach", formData, formData.get("attachement"));
  };
  ...
  return (
    <form>
      ...
      >
        <input {...getInputProps()} />
        <div
          className={`d-flex flex-column align-items-center ${styles.dragArea}`}
        >
          {
            <ArrowUpTrayIcon
        ...
        <ul className="mt-6 flex flex-col">
          {rejected.map(({ file, errors }) => (
            ...
            >
              <div>
                <p
                  className="mt-2  text-sm font-medium flex-wrap"
                  style={{ flexWrap: "wrap", maxWidth: "100px" }}
                >
                  {file.name}
                </p>
